...
// of being clipped. Box styling mirrors the single-line Input used by Item/Qty/Rate.
...
export default function InvoiceLineItemsTable({ lines, onChange }) {
  ...
  const add = type => onChange([...lines, { type, name: "", description: "", quantity: 1, unit_price: 0, taxable: true, source: "Manual" }]);
  return <section className="py-5">
    ...
    <div className="overflow-x-auto rounded-lg border border-gray-800"><table className="w-full min-w-[980px] table-fixed text-sm">
      <colgroup>
      ...
      </tr>)}{lines.length === 0 && <tr><td colSpan="7" className="px-4 py-10 text-center text-gray-500">No line items added.</td></tr>}</tbody>
    </table></div>
  </section>;
}