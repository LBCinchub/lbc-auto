...
export default function InvoiceLineItemsTable({ lines, onChange }) {
  ...
  return <section className="py-5">
    ...
    <div className="overflow-x-auto rounded-lg border border-gray-800"><table className="w-full min-w-[700px] text-sm">
      ...
      <tbody className="divide-y divide-gray-800">{lines.map((line, index) => <tr key={`${line.source || "line"}-${index}`} className="bg-gray-900/40">
        ...
        <td className="px-3 py-2"><Input value={line.description || ""} onChange={e => update(index, "description", e.target.value)} className="border-gray-700 bg-gray-950 text-white" /></td>
        ...
        <td className="px-3 py-2 text-right font-medium text-white">${((Number(line.quantity) || 0) * (Number(line.unit_price) || 0)).toFixed(2)}</td>
        <td className="px-2"><button type="button" onClick={() => onChange(lines.filter((_, i) => i !== index))} className="text-gray-500 hover:text-rose-400"><Trash2 className="h-4 w-4" /></button></td>
