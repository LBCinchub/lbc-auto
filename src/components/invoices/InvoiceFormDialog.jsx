    ...
    base44.auth.me().then(async u => {
      ...
      if (invoice && invoice.id) {
        ...
        setForm(f => ({
          ...
          vehicle_id: sourceEstimate.vehicle_id || "",
          ...
          discount_type: sourceEstimate.discount_type || "none",
          discount_value: sourceEstimate.discount_value || 0,
        }));
                ...
                <tbody className="divide-y divide-gray-800">
                  ...
                  {laborItems.map((row, idx) => (
                    <tr key={idx} className="bg-gray-900">
                      ...
                      <td className="px-2 py-1.5"><Input type="number" onFocus={e => e.target.select()} value={row.hours} onChange={e => updateLabor(idx, "hours", e.target.value)} className="bg-gray-800 border-0 text-white h-8 text-sm text-right" placeholder="1" step="0.5" /></td>
                      <td className="px-2 py-1.5"><Input type="number" onFocus={e => e.target.select()} value={row.rate} onChange={e => updateLabor(idx, "rate", e.target.value)} className="bg-gray-800 border-0 text-white h-8 text-sm text-right" placeholder="120" /></td>
                ...
                <tbody className="divide-y divide-gray-800">
                  ...
                  {partsItems.map((row, idx) => (
                    <tr key={idx} className="bg-gray-900">
                      ...
                      <td className="px-2 py-1.5"><Input type="number" onFocus={e => e.target.select()} value={row.quantity} onChange={e => updatePart(idx, "quantity", e.target.value)} className="bg-gray-800 border-0 text-white h-8 text-sm text-right" placeholder="1" /></td>
                      <td className="px-2 py-1.5"><Input type="number" onFocus={e => e.target.select()} value={row.unit_price} onChange={e => updatePart(idx, "unit_price", e.target.value)} className="bg-gray-800 border-0 text-white h-8 text-sm text-right" placeholder="0.00" /></td>
