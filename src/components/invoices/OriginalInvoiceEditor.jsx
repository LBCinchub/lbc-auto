...
export default function OriginalInvoiceEditor({ source, onClose, onSaved, closeAfterSave = false }) {
  ...
  return <>
    ...
    <div className="max-h-[72vh] overflow-y-auto px-5 py-5 md:px-6"><InvoiceDetailsFields draft={flow.draft} onChange={flow.setDraft} /><InvoiceLineItemsTable lines={flow.draft.line_items || []} onChange={line_items => flow.setDraft({ ...flow.draft, line_items })} /><QuickNotesEditor value={flow.draft.customer_note} onChange={v => flow.setDraft({ ...flow.draft, customer_note: v })} /><InvoiceTotalsSection draft={flow.draft} totals={flow.totals} onChange={flow.setDraft} />{flow.error && <p className="mt-4 rounded-lg border border-rose-500/30 bg-rose-500/10 p-3 text-sm text-rose-300">{flow.error}</p>}</div>
