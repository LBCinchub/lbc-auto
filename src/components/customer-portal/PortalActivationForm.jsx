...
export default function PortalActivationForm({ form, setForm, requested, error, notice, loading, onRequest, onSetup, onBack }) {
  ...
  return (
    <form onSubmit={requested ? onSetup : onRequest} className="space-y-4">
      ...
      <button disabled={loading} className="w-full rounded-lg bg-primary px-4 py-3 font-semibold text-primary-foreground disabled:opacity-50">{loading ? "Please wait…" : requested ? "Create personal passcode" : "Activate Portal Access"}</button>
    </form>
  );
}