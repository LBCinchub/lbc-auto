...
export default function ScannerAiMessage({ message, isLatest, onRepairOrder, onEstimate, onPrint, onSave }) {
  ...
  return <div className={assistant ? "flex justify-start" : "flex justify-end"}>
    <div className={`max-w-[92%] rounded-xl px-3 py-2 text-sm whitespace-pre-wrap leading-relaxed ${assistant ? "bg-gray-800 text-gray-200" : "bg-sky-600 text-white"}`}>
      ...
      {assistant && isLatest && <div className="grid grid-cols-2 gap-2 mt-3 pt-3 border-t border-gray-700">
        <Button size="sm" onClick={onRepairOrder} className="bg-sky-600 hover:bg-sky-700"><Wrench className="w-3.5 h-3.5" /> Add To Repair Order</Button>
        <Button size="sm" onClick={onEstimate} className="bg-emerald-600 hover:bg-emerald-700"><FileText className="w-3.5 h-3.5" /> Create Estimate</Button>
